...
export class BoardService{
    ...
    async create(data: CreateBoardDto){
      ...
      if (!user) throw new Error(`Admin ID with id is ${data.admin} does not exist`);
      else{
    ...
    async remove(id: string) {
        ...
        if (!board) throw new Error(`Project Board with id is ${id} does not exist`);
    ...
    async addMember(idMember: string, idBoard: string){      
      ...
      if (!member) throw new Error(`Member with id is ${idMember} does not exist`);
      else{
        ...
        if (!board) throw new Error(`Project Board with id is ${idBoard} does not exist`);
        else{
